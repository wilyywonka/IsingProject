module IsingParameters
    ...
    type :: param_t
      ...
      real        :: T, dT, OldEnergy, NewEnergy, J, maxT, TotalEnergy, kB
    ...
    subroutine Initialize(Parameters,numTempsSet, numSpinXSet, numSpinYSet, maxTSet, JSet)
      ...
      Parameters%TotalEnergy = 0

      Parameters%kB = 1.38e-23
  ...
  module IsingFunctions
    ...
    subroutine CalculateTotalEnergy(Parameters)
      implicit none
      type(param_t), intent(inout) :: Parameters
      ...
      do IteratorSpinX = 1, Parameters%numSpinX
        do IteratorSpinY = 1, Parameters%numSpinY
          Parameters%TotalEnergy = Parameters%TotalEnergy + CalculateEnergySingle(Parameters,IteratorSpinX,IteratorSpinY)
        end do
      end do
    
    end subroutine CalculateTotalEnergy

    subroutine AddMeasurement(Parameters)
      implicit none
      type(param_t), intent(inout) :: Parameters

      call CalculateTotalEnergy(Parameters)

      Parameters%MagnetizationArray(Parameters%IntT) = Parameters%MagnetizationArray(Parameters%IntT)&
      *((Parameters%EnsembleCounter-1)/Parameters%EnsembleCounter) + Parameters%TotalEnergy*(1/Parameters%EnsembleCounter)
    ...
    function CalculateChance(Parameters) result(chance)
      ...
      if (Parameters%T == 0) then
        chance = 0
      else if (Parameters%NewEnergy<Parameters%OldEnergy) then
        chance = 1
      else
